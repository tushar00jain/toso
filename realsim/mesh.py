...
live inside :class:`~realsim.coordinator.model.ReadCoordinator`, which is shaped
around a single synchronized burst (``run_burst(readers, key)``); capabilities
that are not bursts -- ``kvcache_sim``'s continuous arrival stream -- could not
reuse the coordinator and had to re-derive the wiring underneath it. Extracting
it here lets a capability package hold only capability code: the coordinator is a
burst-shaped consumer of a mesh, and ``kvcache_sim``'s cluster is a KV-shaped one.
...
mesh (the coordinator, which needs the mesh to exist first) can claim it.
"""
