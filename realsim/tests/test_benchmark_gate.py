...
The wider 8B case (``K=290 T=16 G=64``) is one measurement point large enough to
be noise-robust and small enough to stay under a few seconds; its deterministic
outputs are pinned to a JSON baseline. A change that moves the candidate counts
or the tracemalloc peaks or retired instructions fires this test; regenerate the
...
copy the emitted columns into :data:`_BASELINE`::
    ...
    .venv/bin/python -m realsim.tools.benchmark_dedup_control \\
        --keys 290 --source-ranks 16 --generators 64 --warmups 0 --repeats 1
"""
...
_BASELINE = Path(__file__).parent / "baselines" / "dedup_wider_8b.json"
...
def test_wider_8b_matches_baseline():
