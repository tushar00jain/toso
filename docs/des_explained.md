...
| Decision-maker | `DedupPolicy` (a real `Policy`, consulted in the controller) | the scheduler's `select(request, me)` (a `Placement`, asked as its own service), which delegates only "which peer" to a source `Policy` (`LongestPrefixPolicy` by default) |
