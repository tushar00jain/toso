...
| `Arrivals` | `data/serving.py` | The harness-facing `DataPlane`, and almost nothing: it delivers each request to the host it *lands* on. Where a request lands is a load balancer's answer (the run wires client affinity), and where it should *run* is the coordinator's — neither is a serving decision. |
