...
| `LongestPrefixKeySelector`, `LocalOnly`, `ByBatch`, `RoutedPull` | `control/_selector.py` | Every ranking a KV routing decision makes, in one file, over two subjects: keys (which peer serves a prefix gap, whether to name one at all, and the pull a fetch was already answered with) and the scheduler's own predicted decode batches (which host decodes). A ranking over candidates is a selector; a comparison or a cost function is not, so the SLO gates and the pull-vs-recompute test stay in the scheduler — and neither is the prefill order, which is the scheduler's own fold over the pool it keyed with the plans it priced. The first of them is the only part of KV routing that is a *store* question, as a real `KeySelector`: key instances by how much of the requested prefix they hold (the fold breaking a tie on the id), at blocks of that run. Wrapping it in `proposed.selector.Balance` appends the reads lately routed at each source as a second dimension, which the scheduler folds with `by_prefix_and_load` — a prefix run docked one block per such read, and no more — so a replicated hot prefix is not always served by whichever holder sorts first — opt-in, and nothing selects it by default. |
