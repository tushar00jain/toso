...
| `LongestPrefixKeySelector` / `SpreadReadsKeySelector` | `control/_source.py` | The only part of KV routing that is a *store* question, as real `KeySelector`s: rank instances by how much of the requested prefix they hold (id tie-break). `SpreadReadsKeySelector` is that ranking with a bounded discount for how much this selector has lately routed at each source, so a replicated hot prefix is not always served by whichever holder sorts first — opt-in, and nothing selects it by default. |
...
| Decision-maker | `DedupKeySelector` (a real `KeySelector`, consulted in the controller) | the scheduler's `select(request, me)` (a `AnySelector`, asked as its own service), which delegates only "which peer" to a source `KeySelector` (`LongestPrefixKeySelector` by default) |
