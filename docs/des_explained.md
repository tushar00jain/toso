...
| `Cluster` | `sim/cluster.py` | Wraps the **real** `Controller` directory (via `realsim`'s `RealControllerAdapter`/`FakeControllerHandle`) plus a real per-instance `LocalClient`. `prefix_lengths(block_keys)` — `{instance → leading blocks held}` — is the scheduler's core query, computed from the real `locate_volumes`. |
