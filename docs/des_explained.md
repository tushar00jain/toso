...
| Component | File | Role |
|---|---|---|
| `Instance`, `Request`, prefix-hash | `control/request.py`, `workload/_generator.py` | An `Instance` owns one KV pool. A prompt is chunked into fixed `B`-token blocks, each **content-addressed by a prefix-hash chain** so shared leading blocks get identical keys (dedup/prefix-reuse falls out for free). A `Request` carries that chain; the generator of the prompts it addresses builds it. |
| `KVStore` | `data/store.py` | What a KV block is stored as, and the verbs that move it (`publish` / `reuse` / `fetch`), over a `realsim.mesh.Mesh`, which supplies the **real** `Controller` directory plus a real per-instance `LocalClient`. Constructing one checks the premise every fetch is priced against: the carrier is the size the model predicts. Eviction is not here — a volume drops its own coldest and tells the directory itself. |
...
| `DecodeEngine` | `data/_decode.py` | Batched, stepped decode on the async engine (makes **TBT real**). Emits one token per step per batched request; step time = `decode_step_time(batch)`. Models the VRAM `max_batch` cap (over-cap requests queue, their wait counting against TBT) and **owns** the per-instance compute timeline. |
...
| `ServingHost` | `data/serving.py` | One serving instance: its cache, its decode batch, its compute. Plays two roles — **router** (a request that lands here is routed by the coordinator, then served here or forwarded to the host named) and **server** (queue wait → real prefix pull → prefill charge → publish → decode handoff → decode-done, recording TBT). Also owns **prefill/decode coupling**, which is a deployment fact, not a policy: when coupled it applies each plan's reservation to its decode timeline and reports each step's end back to the scheduler. Reaches other hosts only through `peers`, so a forward and a decode handoff are charged hops. |
| `_Arrivals` | `workload/_serving.py` | The harness-facing `DataPlane`, and almost nothing: it delivers each request to the host it *lands* on. Where a request lands is a load balancer's answer (the run wires client affinity), and where it should *run* is the coordinator's — neither is a serving decision, so this is run wiring rather than capability code: a deployment deletes it and keeps the hosts. |
