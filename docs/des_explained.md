...
| Type | What it is |
|---|---|
| `KeySelector.select(keys, requester)` | Which volume serves these keys for this requester, and **when** it is usable — ranked sources, plus a gate the plane spends before the answer travels, so a caller reads only from sources already usable. Naive — every holder, directory order — is the default, and is exactly what the real directory answers unaided. Reached as a service of its own: the data plane asks it and hands the answer to an ordinary `client.get` as a source preference (`prefer()`), so nothing is installed in the store and an app that wants to *price* alternatives asks the same object itself. |
