...
| Component | File | Role |
|---|---|---|
| `Dedup` | `control/routing.py` | The capability's whole control plane, two members: `sources(keys, me)` answers with a source and does not answer until that source is usable, and `published(me, keys)` is how it hears that a put landed. Holds no client, no volume, no mesh — only the chain below. |
| `PlannedPeer`, `HolderRanking` | `control/_selector.py` | The routing decision, as two real `KeySelector` links of a `FirstMatch` chain (with `NaiveKeySelector` as its tail): hand each requester the next peer with a free slot under a fan-out cap, else rank the volumes that already hold it (nearest first, priced by locality tier) so its head opens the tree; gate the answer on the read-through that head owes, and retire one that holds nothing and owes nothing. No burst loop, no reader list, no count of how many readers there will be. |
| `FanoutSensor`, `FanoutView` | `control/_sensor/`, `control/_view.py` | Who is folded in behind whom, which puts are owed and who waits on them: this plane's one `proposed.Sensor`, built by `Dedup.attach` and read by the links through the view they are attached to (`view.fanout`), so no link is handed a sensor and two planes cannot share one. Beside it, `Readiness`: one event per fact still outstanding, opened against the directory and nothing remembered. |
...
| Decision-maker | `Dedup` — one `ControlPlane`, asked `sources(keys, me)`; it ranks with a chain of `KeySelector` utilities (`PlannedPeer`, `HolderRanking`, `NaiveKeySelector`) | one `ControlPlane` too, asked `decide(request, me)` for compute placement and `sources(keys, me)` for the fetch that plan implies; it ranks with a `KeySelector` utility (`LongestPrefixKeySelector` by default) |
