"""Dedup controller's promised-directory overlay and fetch narrowing."""
...
__all__ = ["FetchCoverage", "PlannedFetch", "discover_fetch", "plan_fetch"]
...
def discover_fetch(
    ...
    """Expand live and promised metadata once for selection and materialization."""
    ...
    return FetchCoverage(tuple(by_key))
...
def plan_fetch(
    ...
    *,
    ...
    for key_coverage in coverage.by_key:
        ...
        ranked = [(source, effective[source]) for source in order if source in present]
        choices = ranked or [
        ...
        live = {source.source: Counter(source.live) for source in key_coverage.sources}
