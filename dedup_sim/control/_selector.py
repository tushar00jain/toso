"""Which peer serves a read, and when: the dedup source :class:`~proposed.selector.KeySelector`.
...
:class:`PeerKeySelector` answers differently. Readers ask it in order; the first is
routed to the volume that already holds the key (the single fabric hop), and every
later one is routed to a **peer** -- a reader that is *about to* hold it. Because that
peer has not registered yet, the decision carries a readiness gate, so a requester
does not get its answer until the peer's read-through put lands -- which this selector
hears because the reader that made it says so (:meth:`PeerKeySelector.published`).
Peers are handed out FIFO under a fan-out cap, so cap 1 builds a chain and cap >= 2 a
shallow tree.

Whether a peer holds the key is asked of the directory every time an answer is
formed (:meth:`PeerKeySelector._registered`), never remembered: volumes evict, so a
peer that registered the key and later dropped it for a newer version is a peer the
next requester has to wait for again.
...
selector is already tracking: a requester it routes is going to read the key through,
so from the moment it asks it *owes* that registration, and waiting for it is bounded.
A source that owes nothing has to hold the key already, and one that holds nothing and
owes nothing is no longer a source at all -- it is retired and the directory answers
for that requester (:meth:`PeerKeySelector._retire`).
...
schedule this module runs. Nor is the gate ever spent here: a ranking that stays
inside the plane holding it may carry one, and awaiting it is that plane's business
(:meth:`dedup_sim.control.routing.Dedup.sources`).
"""
...
from proposed import DecisionLog, Key, KeySelector, nearest, Selection

from ._readiness import Readiness

__all__ = ["PeerKeySelector"]
...
class PeerKeySelector(KeySelector[None]):
    """Route each requester to a peer, not the origin.

    ``KeySelector[None]`` -- a ranking with no price, because there is no choice left
    to weigh: a requester is folded in behind one peer, and the alternative to reading
    from it is the origin hop this exists to avoid.
    ...
    Args:
        ...
        trace: optional :class:`~proposed.selector.DecisionLog` to record each
            routing decision into. Changes no metric; this selector behaves
    ...
    def __init__(
        ...
        # a source safe -- see the module docstring.
    ...
    def published(self, requester: str, keys: Sequence[Key]) -> None:
        """``requester``'s read-through has landed: ``keys`` are on its volume now.

        Releases whoever was parked on that volume, and settles the debt -- the
        publication happened, so from here on the directory is what says whether the
        volume still holds the key.

        Not a coroutine: this is the fact arriving, and the plane that carries it in
        (:meth:`dedup_sim.control.routing.Dedup.published`) is where the ordering it
        has to arrive in is stated.
        """
        for key in keys:
