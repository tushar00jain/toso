...
class LRUCache:
    ...
    def __init__(self, capacity: Optional[int] = None) -> None:
        ...
        self._recency: Dict[BlockKey, int] = {}  # key -> last-access clock
        self._clock: int = 0

    def __contains__(self, key: BlockKey) -> bool:
        return key in self._recency

    def held(self) -> Set[BlockKey]:
    ...
    def touch(self, keys: List[BlockKey]) -> None:
    ...
    def admit(self, keys: List[BlockKey]) -> List[BlockKey]:
        """Insert/refresh ``keys``; evict the coldest until within capacity.

        Returns the list of evicted keys (so the caller can ``notify_delete`` them
        from the index). Eviction ties break on the block key for determinism.
        """
        for k in keys:
            self._clock += 1
            self._recency[k] = self._clock
