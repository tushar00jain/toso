...
* :mod:`proposed.selector` -- :class:`~proposed.selector.Selection` and the selection
  ...
  combinators -- :class:`~proposed.selector.FirstMatch` (try selectors in order),
  :class:`~proposed.selector.KeySelectorChain` (the same over keys) and
  :class:`~proposed.selector.Refine` (one selector's ranking, narrowed by the tests
  behind it) -- and the :class:`~proposed.selector.Selector` base they are typed on are
...
* :mod:`proposed.deployment` -- :class:`~proposed.deployment.Controller`, the
  ...
  same read with nothing applied, which is what a sensor reads. Beside it,
