...
* :class:`ControlPlane` -- the deciding half's lifecycle: knobs at construction,
  ...
  harvests off it once attached -- where its hosts report their facts
  (:attr:`ControlPlane.dispatcher`), to put a service in front of. **One per
...
class DataPlane(Actor):
    ...
    def attach(self, deployment: Any) -> None:
        ...
        plane it asks and the dispatcher it reports into do not exist until the
...
class ControlPlane(Sensing, Actor):
    ...
    #: Where this plane's hosts report their facts. Read after :meth:`attach` by
    #: whoever assembles the run, and given a service of its own so the application's
    #: hosts can report without a question being asked. Singular because the seam is:
    #: a plane may hold several sensors -- its own bookkeeping among them -- and one
    #: action moves every one of them a fact concerns, so what is fronted is the
    #: :class:`~proposed.dispatch.Dispatcher` and never a sensor. ``None`` -- the
