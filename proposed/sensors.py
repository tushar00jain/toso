...
from typing import Any, Dict, FrozenSet, Iterator, Mapping, Optional, Sequence, Tuple, TypeVar
...
class DirectorySensor(Sensor):
    """One coherent live directory read."""
    ...
    def locate_live(self, keys: Sequence[str]) -> Dict[str, Dict[str, Any]]:
        if not keys:
            return {}
        return self.directory._locate(list(keys), missing_ok=True)
