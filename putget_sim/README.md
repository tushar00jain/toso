...
|---|---|---|---|
| `control/` — what is decided | **absent** — no preference is passed, so the directory's own order stands | `routing.py` + `_selector.py` + `_sensor/` + `_view.py`: a `KeySelector` chain behind one plane — a ranked source, once it is usable | `scheduler.py` + `_answer.py` + `_selector.py` + `_sensor/` + `_view.py` |
