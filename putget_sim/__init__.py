...
selector and no data plane, so every reader locates the origin and pulls from it:
...
* :mod:`putget_sim.workload` -- the burst itself: a topology, an allocation-free
  ...
  with no selector and no coordinator in it, plus the single
...
:class:`~proposed.selector.KeySelector` and a :class:`~proposed.plane.DataPlane` is the
